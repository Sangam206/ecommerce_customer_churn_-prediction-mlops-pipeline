...
FILE_OK_GAUGE       = Gauge("model_file_ok",      "model file healthy (1=ok, 0=error)")
FILE_CHANGED_GAUGE  = Gauge("model_file_changed", "model file changed since last run (1=yes)")
...
def check_model_file(path: Path) -> bool:
    """Check existence and corruption of model file."""
    if not path.exists():
        logger.error(f"[ALERT] model file MISSING at {path}")
        return False

    size_kb  = path.stat().st_size / 1024
    modified = datetime.fromtimestamp(path.stat().st_mtime)
    logger.info(f"model file | Size: {size_kb:.1f} KB | Last Modified: {modified}")

    try:
        with open(path, "rb") as f:
            pickle.load(f)
        logger.info("model file — Loads successfully. No corruption.")
    except Exception as e:
        logger.error(f"[ALERT] model file CORRUPTED: {e}")
        return False
...
def detect_model_change(path: Path) -> bool:
    """Detect if model file changed since last run via hash comparison."""
    ...
    if HASH_LOG.exists():
        ...
        if not last.empty:
            ...
            if current_hash != prev_hash:
                logger.warning(
                    f"[MODEL CHANGED] model file hash changed! "
                    f"Previous: {prev_hash[:8]}... → Current: {current_hash[:8]}... "
                    f"Model was retrained or replaced."
                )
                changed = True
            else:
                logger.info("model file — Hash unchanged. Same model as last run.")
    ...
    logger.info(f"model file — Current hash: {current_hash[:8]}... logged.")
...
def run_evidently_report(reference: pd.DataFrame, current: pd.DataFrame, name: str) -> float:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"{name} — Could not extract drift score.")

        return drift_score

    except Exception as e:
        logger.error(f"Failed Evidently report for {name}: {e}")
        return 0.0
...
def run_monitoring():
    ...
    if not file_ok:
        logger.error("Model file is missing or corrupted. Aborting.")
        return
    ...
    logger.info(f"F1        : {metrics['f1_macro']}")
